...
router.post('/login',[
    ...
    try {
        ...
        if(!user){
            return res.status(400).json({error : 'Kindly be precised while entering your data'});
        }

        if(!(await bcrypt.compare(password,user.password))){
            return res.status(400).json({error : 'Kndly be precised while entering your credentials'});
        } 
        const data = {
            user:{
                id : user.id
